...
<div class="centercontent">
    ...
    <div id="contentwrapper" class="contentwrapper users-car">
        <form method="post" action="" id="submitForm">
            ...
            <input type="hidden" name="oaLeave.departAuditTime" value="<fmt:formatDate value="${nowDate}" pattern="yyyy-MM-dd HH:mm:ss"></fmt:formatDate>">
            <input type="hidden" name="oaLeave.leaveDays" value="${oaLeave.leaveDays}">
            <div class="testtle-tables">
                <table border="1">
                    ...
                    <tr>
                        <td class="pt" rowspan="2"><small class="c-red">*</small>部门负责人审批意见</td>
                        <td class="pt" rowspan="2" style="border-right: none;color:#666;"><textarea cols="30" rows="3" name="oaLeave.departmentOption" id = "departmentOption">${oaLeave.departmentOption}</textarea></td>
                ...
                <c:if test="${oaLeave.leaveDays>2}">
                    <a id="chooseBtn" class="submit radius2" onclick = "chooseUser()" style="cursor: pointer">选择分管副校长审批</a>
                </c:if>
...
<script type = "text/javascript">
    ...
    var id = '${oaLeave.id}';
    var type = 2;
    var leaveDays='${oaLeave.leaveDays>2}';
    initSignature(id, "部门领导审批");
    function addFormSubmit(flag) {
        if (!jQuery("#departmentOption").val()) {
            alert("部门负责人意见不能为空");
            return;
        }
        if(flag==0 && leaveDays=='true' && !jQuery("#userIds").val()){
            alert('请选择分管副校长审批');
            return;
        }
