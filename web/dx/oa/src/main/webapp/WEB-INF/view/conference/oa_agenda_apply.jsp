        ...
        <div id="basicform" class="subcontent">
            <form class="stdform stdform2" method="post" action="" id="saveForm">
                <p>
                    ...
                    <span class="field">
                        <input style="width:60px;" type="text" name="agenda.year" class="longinput"
                               value="<fmt:formatDate value="${nowDate}" pattern="yyyy"/>"> 年第
                        <input style="width:40px;" type="text" name="agenda.frequency" class="longinput"> 次
                    </span>
                </p>
                <p>
                ...
                <p>
                    ...
                    <span class="field">
                        <%--<input type="text" name="agenda.record" class="longinput">--%>
                ...
                <input type="hidden" name="agenda.absent" id="absent">
                <input type="hidden" name="agenda.compere" id="compere">
                <input type="hidden" name="agenda.record" id="record">
                <input type="hidden" name="agenda.topicIds" id="topicIds">
                <input type="hidden" name="agenda.location" id="locationName">
                <input type="hidden" name="agenda.locationId" id="locationId" value="0">
                <%--使用结束时间--%>
                <input type="hidden" id="turnTime">
                <%--使用开始时间--%>
                <input type="hidden" id="useTime">
                <input type="hidden" name="type" id="type">
                <input type="hidden" value="<fmt:formatDate value="${nowDate}" pattern="yyyy-MM-dd HH:mm:ss"/>" name="agenda.startTime"/>
                <p class="stdformbutton" style="text-align: center">
                    <button class="submit radius2" onclick="addFormSubmit();return false;">提 交</button>&nbsp;&nbsp;&nbsp;&nbsp;
                    <button class="submit radius2" onclick="saveDraftFormSubmit();return false;">存草稿</button>
                </p>
            </form>
            <br/>
        </div>
...
//            }
    ...
    function saveDraftFormSubmit() {
        ...
        var params = jQuery("#saveForm").serialize();
        jQuery.ajax({
            url: '${ctx}/admin/oa/agenda/process/saveOaMeetingAgendaDraft.json',
            data: params,
            type: 'post',
            dataType: 'json',
            success: function (result) {
...
//            jQuery(obj).stopPropagation()
    ...
    <%--function chooseTopic() {--%>
        ...
        <%--jQuery.ajax({--%>
            ...
            <%--dataType: 'html',--%>
            <%--success: function (result) {--%>
                <%--jQuery.alerts._show("选择会场", result, null, 'dialog', function (data) {--%>
                    <%--if (!data) {--%>
                        <%--if (len < 0) topicNames.children('button').remove();--%>
