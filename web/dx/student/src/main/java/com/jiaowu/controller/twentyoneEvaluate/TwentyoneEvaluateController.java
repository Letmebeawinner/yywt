...
public class TwentyoneEvaluateController extends BaseController {
    ...
    public String toCreateTwentyoneEvaluate(HttpServletRequest request, @RequestParam(value = "type", required = false) Integer type) {
        try {
            ...
            if (userMap.get("userType").equals("3")) {
                ...
                Map<String, String> teacher = hrHessianService.queryEmployeeById(Long.parseLong(userMap.get("linkId")));
                List<Classes> classesList = classesBiz.find(null, " status=1 and ( teacherId=" + teacher.get("id") + " or deputyTeacherId=" + teacher.get("id") + " )");
    ...
    public Map<String, Object> createTwentyoneEvaluate(HttpServletRequest request, TwentyoneEvaluate twentyoneEvaluate) {
        ...
        try {
            Map<String, String> userMap = SysUserUtils.getLoginSysUser(request);
            if (!userMap.get("userType").equals("3") && !userMap.get("userType").equals("2")) {
                return this.resultJson(ErrorCode.ERROR_PARAMETER_VERIFY, "您没有权限填写21项测评", null);
            }
            ...
            if (userMap.get("userType").equals("3")) {
                ...
                List<TwentyoneEvaluate> twentyoneEvaluateList = twentyoneEvaluateBiz.find(null, " status=1 and userType=1 and userId=" + user.getId() + " and classId=" + classId);
                if (twentyoneEvaluateList != null && twentyoneEvaluateList.size() > 0) {
                    return this.resultJson(ErrorCode.ERROR_PARAMETER_VERIFY, "您在当前班次已填写过21项测评,不能重复提交。", null);
                }
                Classes classes = classesBiz.findById(classId);
                if (CheckTime(classes))
                    return this.resultJson(ErrorCode.ERROR_PARAMETER_VERIFY, "暂不可提交，班次结束前一周后可提交。", null);
                twentyoneEvaluate.setClassId(classId);
                twentyoneEvaluate.setUserId(user.getId());
                twentyoneEvaluate.setUserName(user.getName());
                twentyoneEvaluate.setUserType(1);
            } else if (userMap.get("userType").equals("2")) {
                Map<String, String> teacher = hrHessianService.queryEmployeeById(Long.parseLong(userMap.get("linkId")));
                List<Classes> classesList = classesBiz.find(null, " status=1 and ( teacherId=" + teacher.get("id") + " or deputyTeacherId=" + teacher.get("id") + " )  order by id desc");
                if (classesList == null || classesList.size() <= 0) {
                    return this.resultJson(ErrorCode.ERROR_PARAMETER_VERIFY, "您没有权限填写21项测评", null);
                }
                Long classId = classesList.get(0).getId();
                List<TwentyoneEvaluate> twentyoneEvaluateList = twentyoneEvaluateBiz.find(null, " status=1 and userType=2 and userId=" + teacher.get("id") + " and classId=" + classId);
                if (twentyoneEvaluateList != null && twentyoneEvaluateList.size() > 0) {
                    return this.resultJson(ErrorCode.ERROR_PARAMETER_VERIFY, "您在当前班次已填写过21项测评,不能重复提交。", null);
                }
                Classes classes = classesBiz.findById(classId);
                if (CheckTime(classes))
                    return this.resultJson(ErrorCode.ERROR_PARAMETER_VERIFY, "暂不可提交，班次结束前一周后可提交。", null);
                twentyoneEvaluate.setClassId(classId);
                twentyoneEvaluate.setUserId(Long.valueOf(teacher.get("id")));
                twentyoneEvaluate.setUserName(teacher.get("name"));
                twentyoneEvaluate.setUserType(2);
            }
            twentyoneEvaluateBiz.save(twentyoneEvaluate);
            json = this.resultJson(ErrorCode.SUCCESS, "提交成功", null);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private boolean CheckTime(Classes classes) {
        if (classes == null) {
    ...
    public String twentyoneEvaluateList(HttpServletRequest request, TwentyoneEvaluate twentyoneEvaluate,
        ...
        try {
            ...
            if (!StringUtils.isTrimEmpty(twentyoneEvaluate.getUserName())) {
                whereSql += " and userName like '%" + twentyoneEvaluate.getUserName() + "%'";
            }
            if (twentyoneEvaluate.getClassId() != null && twentyoneEvaluate.getClassId() > 0) {
                whereSql += " and classId = " + twentyoneEvaluate.getClassId();
            }
