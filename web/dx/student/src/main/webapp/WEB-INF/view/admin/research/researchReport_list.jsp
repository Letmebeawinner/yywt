	...
	<body>
	    <div class="centercontent tables">
	         <div class="pageheader notab" style="margin-left: 30px">
                 <h1 class="pagetitle"><c:if test="${researchReport.approvalDepartment == 1}">科研处</c:if><c:if
                         test="${researchReport.approvalDepartment == 2}">生态所</c:if>调研报告</h1>
                <span>
                ...
                <div class="overviewhead clearfix mb10">
                    <div class="fl mt5">
                        <form class="disIb" id="searchForm"
                              action="${ctx}/admin/jiaowu/researchReport/researchReportList.json"
                              method="get">
                            <input type="hidden" name="approvalDepartment" value="${researchReport.approvalDepartment}">
                            <input type="hidden" id="researchId" name="researchReport.researchId"/>
                            <input type="hidden" id="researchName" name="researchReport.researchName"/>
                            ...
                            <div class="disIb ml20 mb10">
                                <span class="vam">第一执笔人 &nbsp;</span>
                                <label class="vam">
                                    <input type="text" name="researchReport.zbr" value="${researchReport.zbr}"
                                           placeholder="请输入第一执笔人">
                                </label>
                            </div>
                    ...
                    <table cellpadding="0" cellspacing="0" border="0" class="stdtable">
                        <thead>
                            <tr>
                                ...
                                <th class="head0 center" width="20%">第一执笔人</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>${researchReport.zbr}</td>
                                <td>
                                   <c:if test="${researchReport.type=='student'}">学员</c:if>
                                   <c:if test="${researchReport.type=='teacher'}">老师</c:if>
                                </td>
                                <td>
                                    <c:if test="${researchReport.audit == 0}">未审批</c:if>
                                    <c:if test="${researchReport.audit == 1}">初审已通过</c:if>
                                    <c:if test="${researchReport.audit == 2}">初审未通过</c:if>
                                    <c:if test="${researchReport.audit == 3}">评审已通过,
                                        <c:if test="${researchReport.assessmentLevel == 1}">课题等级为: 优</c:if>
                                        <c:if test="${researchReport.assessmentLevel == 2}"> 课题等级为: 良</c:if>
                                        <c:if test="${researchReport.assessmentLevel == 3}">课题等级为: 合格</c:if>
                                        <c:if test="${researchReport.assessmentLevel == 4}">课题等级为: 不合格</c:if>
                                    </c:if>
                                    <c:if test="${researchReport.audit == 4}">评审未通过</c:if>
                                </td>
                                <td><fmt:formatDate type="both" value="${researchReport.createTime}" pattern="yyyy-MM-dd HH:mm"/></td>                                
                                <td class="center">
