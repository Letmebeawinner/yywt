...
<%-- 		<script type="text/javascript" src="${ctx}/static/js/custom/elements.js"></script> --%>
        ...
        function replyToTopic(){
        	...
        	jQuery.ajax({
	    		url:'${ctx}/admin/jiaowu/reply/addReplyToTopic.json',
	    		data:{"topicId":topicId,"content":replyContent},
	    		type:'post',
	    		dataType:'json',		
	    		success:function (result){
	    			if(result.code=="0"){
...
// 						window.location.reload();
        ...
        function addPraise(replyId){
        	...
        	jQuery.ajax({
	    		url:'${ctx}/admin/jiaowu/reply/addPraise.json',
...
// 	    			alert("点赞失败,请稍后再试!");
     ...
     jQuery.ajax({
         url:'${ctx}/admin/jiaowu/reply/getChildReply.json',
         data:{"replyId":replyId},
         type:'post',
         dataType:'json',        
         success:function (result){
   ...
   var topicId="${topic.id}";
   jQuery.ajax({
       url:'${ctx}/admin/jiaowu/reply/addChildReply.json',
       data:{"topicId":topicId,"replyId":replyId,"content":replyContent},
       type:'post',
       dataType:'json',        
       success:function (result){
           if(result.code=="0"){
	    ...
	    <div class="centercontent tables">
	         <div class="pageheader notab" style="margin-left: 30px">
	         	<span>
					...
					1.本页面展示一个话题和所有用户评论的内容.<br>
					2.用户登录后,可在下方的输入框对话题进行评论.<br>
					3.用户登录后,可点击某评论下方的点赞图片,对评论点赞,只可点赞一次.<br>
					4.用户登录后,可点击某评论下方的评论图片,对某评论回复,可回复多次.<br>
                </span>
                <h1 class="pagetitle">${topic.title}</h1>
<!--                 <h6 ></h6> -->
             ...
             <div id="contentwrapper" class="contentwrapper">
                ...
                <div class="statusbox">
                            ...
                            <div class="submit"><button class="stdbtn btn_yellow" onclick="replyToTopic()">回复</button></div>
                </div>
            	<!-- 主要内容结束 -->
				<div class="clear"></div>
            </div><!-- #updates -->
