	    ...
	    <div class="centercontent tables">
	         <div class="pageheader" style="padding-left: 30px">
                <h1 class="pagetitle">管理员用户列表</h1>
                 <span>
                    ...
                    4. 锁定、恢复：点击<span style="color:red">锁定</span>按钮，锁定用户,锁定的用户将不能进行登录等操作，点击<span  style="color:red">恢复</span>恢复用户， 用户恢复正常<br>
             ...
             <div id="contentwrapper" class="contentwrapper">
                ...
                <div class="overviewhead clearfix mb10">
                    <div class="fl mt5">
                    <form  class="disIb" id="searchForm" action="${ctx}/admin/base/sysuser/queryUserList.json" method="get">
                       ...
                       <div class="disIb ml20 mb10">
                           ...
                           <label class="vam">
                            <input style="width: auto;" type="text" class="hasDatepicker" name="check" placeholder="用户名称/邮箱/手机号" value="${check}">
                           </label>
                       </div>
                        <div class="tableoptions disIb mb10 ml20">
                            <span class="vam">用户类型 &nbsp;</span>
                            <label class="vam">
                                <select name="userType" class="vam" id="userType">
                                    <option value="">--请选择--</option>
                                    <option value="1" <c:if test="${userType==1}">selected</c:if>>管理员</option>
                                    <option value="2" <c:if test="${userType==2}">selected</c:if>>教职工</option>
                                    <option value="3" <c:if test="${userType==3}">selected</c:if>>学员</option>
                                    <option value="4" <c:if test="${userType==4}">selected</c:if>>驾驶员</option>
                                    <option value="5" <c:if test="${userType==5}">selected</c:if>>单位报名</option>
                                    <option value="6" <c:if test="${userType==6}">selected</c:if>>物业公司</option>
                                </select>
                            </label>
                        </div>
                       </form>
                    ...
                    <table cellpadding="0" cellspacing="0" border="0" class="stdtable">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="center">
                                    <a class="stdbtn" title="编辑" href="/admin/base/sysuser/toUpdateSysUser.json?id=${sysuser.id}"><span>编辑</span></a>
                                    <c:if test="${sysuser.userType!=3}">
                                    <a href="javascript:void(0)" class="stdbtn" title="编辑"  onclick="showSysUserDepartment(${sysuser.id})">设置部门</a>
                                    </c:if>
                                        <a href="javascript:void(0)" class="stdbtn" title="编辑"  onclick="showSysUserRole(${sysuser.id})">设置角色</a>
                                    <a href="javascript:void(0)" class="stdbtn" title="编辑"  onclick="deleteSysUser(${sysuser.id})">删除</a>
                                    <c:if test="${sysuser.status==1}">
                                       <a href="javascript:void(0)" class="stdbtn" title="查看" onclick="updateStatus(${sysuser.id},this)">恢复</a>
                                   </c:if>
                                    <c:if test="${sysuser.status==0}">
                                        <a href="javascript:void(0)" class="stdbtn" title="删除" onclick="updateStatus(${sysuser.id},this)">锁定</a>
                                    </c:if>
