...
<head>
    ...
    <script type="text/javascript">
        ...
        function doMerge() {
            if (jQuery("input[name='result.archiveNo']").val().length === 0) {
                alert("请输入档号")
                return false
            }
            ...
            var params = jQuery("#mergeForm").serialize();
            jQuery.ajax({
                url: "${ctx}/admin/ky/doMergeResult.json",
                data: params,
                type: "post",
                dataType: "json",
                success: function (data) {
                    alert(data.message)
                    if (data.code === "0") {
                        window.location.href = "${ctx}/admin/ky/getResultFileList.json?queryResult.resultType=" + data.data.resultType
                            + "&queryResult.resultForm=" + data.data.resultForm;
                    }
                }
            })
        }
...
<div class="centercontent">
    ...
    <div id="contentwrapper" class="contentwrapper">
        <div id="basicform" class="subcontent">
            <form class="stdform stdform2" method="post" action="" id="mergeForm">

                <%--<p>--%>
                <%--<label><em style="color: red;">*</em>档号</label>--%>
                <%--<span class="field"><input type="text" value="${result.archiveNo}" name="result.archiveNo"--%>
                ...
                <p id="uploadArgumentTheory" style="display: none">
                    <label>上传论证活页</label>
                    <span class="field">
                        <input type="text" name="result.fileUrlTheory" value="${result.fileUrlTheory}"
                               id="fileUrlTheory"/>
                ...
                <p>
                    ...
                    <span class="field">
                        <input type="hidden" name="result.fileUrl" value="${result.fileUrl}" id="fileUrl"/>
                ...
                <p class="stdformbutton" style="text-align: center">
                    <button class="submit radius2" onclick="doMerge();return false;">修 改</button>
                    <input type="reset" class="reset radius2" value="返 回" onclick="javascript:history.go(-1);"/>
                </p>

                <input type="hidden" name="result.id" id="id" class="longinput" value="${result.id}"/>
                <input type="hidden" name="result.resultType" class="longinput" value="${result.resultType}"/>
