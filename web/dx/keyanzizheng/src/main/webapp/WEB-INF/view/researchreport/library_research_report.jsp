        ...
        <div class="overviewhead clearfix mb10">
            <div class="fl mt5">
                <form class="disIb" id="searchForm"
                      action="${ctx}/admin/ky/rr/researchReportLibrary.json"
                      method="get">
                ...
                <c:if test="${not empty rqList}">
                    <c:forEach items="${rqList}" var="rql" varStatus="status">
                        <tr>
                            ...
                            <td>
                                <fmt:formatDate type="both" value="${rql.updateTime}" pattern="yyyy-MM-dd HH:mm"/>
                            </td>
                            <td class="center">

                                <c:if test="${rql.archive == 0}">
                                    <a href="${ctx}/admin/ky/rr/researchReportFiled.json?rqId=${rql.id}" class="stdbtn"
                                       title="归档">归档</a>
                                </c:if>

                                <c:if test="${rql.archive == 1}">
                                    <a href="${ctx}/admin/ky/rr/detailArchive.json?oaArchiveId=${rql.oaArchiveId}"
                                       class="stdbtn" title="查看档案">查看档案</a>
                                </c:if>
