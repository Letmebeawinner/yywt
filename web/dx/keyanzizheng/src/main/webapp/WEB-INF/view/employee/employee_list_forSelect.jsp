...
<div class="tables">
    <div class="pageheader notab">
        <h1 class="pagetitle">教师列表</h1>
    </div><!--pageheader-->
    <div id="contentwrapper" class="contentwrapper">
        <!-- 搜索条件，开始 -->
        <div class="overviewhead clearfix mb10">
            <div class="fl mt5">
                <form class="disIb" id="searchForm" action="${ctx}/jiaowu/teacher/teacherListForSelect.json"
                      method="get">
                    ...
                    <div class="disIb ml20 mb10">
                        ...
                        <label class="vam">
                            <input id="employeeNo" style="width: auto;" name="teacher.employeeNo" type="text"
                                   class="hasDatepicker" value="${teacher.employeeNo}" placeholder="请输入编号">
                        </label>
                    </div>
            ...
            <table cellpadding="0" cellspacing="0" border="0" class="stdtable">
                ...
                <c:if test="${teacherList!=null&&teacherList.size()>0 }">
                    <c:forEach items="${teacherList}" var="teacher">
                        <tr>
                            <td><input type="radio" value="${teacher.id}-${teacher.name}" class="questionIds"
                                       name="teacherId"/>${teachingComment.id }</td>
                            <td>${teacher.name}</td>
                            <td>${teacher.birthDay}</td>
                            <td>${teacher.employeeNo}</td>
                            <td>${teacher.employeeTypeName}</td>
                            <td>
                                <c:if test="${teacher.sex==1}">男</c:if>
                                <c:if test="${teacher.sex==2}">女</c:if>
                            </td>
                            <td>${teacher.mobile}</td>
                            <td>${teacher.identityCard}</td>
                            <td>${teacher.nationality}</td>
                            <td>${teacher.baseMoney}</td>
                        </tr>
                    </c:forEach>
                </c:if>
                </tbody>
