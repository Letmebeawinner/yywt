            ...
            <table cellpadding="0" cellspacing="0" border="0" class="stdtable">
                ...
                <c:if test="${courseList!=null&&courseList.size()>0}">
                    <c:forEach items="${courseList}" var="course">
                        <tr>
                            <td><input name="courseId" type="checkbox" value="${course.id}-${course.name}" class="questionIds" />${course.id }</td>
                            <td>${course.name}</td>
                            ...
                            <td><fmt:formatDate type="both" value="${course.createTime}" pattern="yyyy-MM-dd HH:mm"/></td>
                                <%--  <td class="center">
                                     <a href="${ctx}/admin/jiaowu/course/toUpdateClass.json?id=${course.id}" class="stdbtn" title="修改">修改</a>
                                     <a href="" onclick="deleteCourse(${course.id})" class="stdbtn" title="删除">删除</a>
                                 </td> --%>
                        </tr>
                    </c:forEach>
                </c:if>
