...
//		var existObjectArray=new Array();
		jQuery.ajax({
    		url:'ajaxClassroomCourseArrange.json',
    				...
    				if(courseArrangeList!=null&&courseArrangeList.length>0){
    					for(var i=0;i<courseArrangeList.length;i++){
    						...
    						var existObject={
                                ...
                                " 开始时间:" + courseArrangeList[i].startTime+
                                " 结束时间" + courseArrangeList[i].endTime,
...
//    						existObjectArray.push(existObject);
		...
		jQuery('#calendar').fullCalendar({
            ...
            eventClick: function(calEvent, jsEvent, view) {
                if(calEvent.id!==0){
                    window.location.href='/admin/jiaowu/teach/updateTeachingProgramCourse.json?id='+calEvent.url;
                }
                return false;
            }
		});
