    ...
    <div id="contentwrapper" class="contentwrapper">
        <div id="basicform" class="subcontent">
            <form class="stdform stdform2" method="post" action="" id="addinstitution">
                <p>
                    <input type="hidden" name="institution.id" value="${institution.id}" id="instituId" class="longinput"/>
                    <input type="hidden" name="institution.employeeId" value="${institution.employeeId}" id="employeeId" class="longinput"/>
                    <label><em style="color: red;">*</em>获奖名称</label>
                    <span class="field"><input type="text" name="institution.title" id="title" value="${institution.title}"
                                               class="longinput"/></span>
                </p>
                <p>
                    <label><em style="color: red;">*</em>颁奖单位</label>
                    <span class="field"><input type="text" name="institution.unit" id="unit" class="longinput" value="${institution.unit}"/></span>
                </p>
                <p>
                    <label><em style="color: red;">*</em>是否发放证书</label>
                    <span class="field">
                        <input type="radio" name="institution.isCertificate"  value="0" <c:if test="${institution.isCertificate==0}">checked</c:if>/>是&nbsp;&nbsp;
                        <input type="radio" name="institution.isCertificate" value="1" <c:if test="${institution.isCertificate==1}">checked</c:if>/>否&nbsp;
                    </span>
                </p>
                <p>
                    <label><em style="color: red;">*</em>证书时间</label>
                    <span class="field"><input type="text" name="institution.certificateTime" id="certificateTime" value="<fmt:formatDate value="${institution.certificateTime}" pattern="yyyy-MM-dd"/>"
                                               class="longinput"/></span>
                </p>
                <p>
