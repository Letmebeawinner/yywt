            ...
            jQuery.alerts._show('选择教职工',null, null,'addCont',function (confirm) {
                if(confirm){
                    if(employeeIds!=""){
                        jQuery.ajax({
                            type:"post",
                            url:"/admin/rs/addCountrysideEmployee.json",
...
function addcountrysideFormSubmit() {
    ...
    jQuery.ajax({
        url: "/admin/rs/addCountryside.json",
        data: date,
        type: "post",
        dataType: "json",
        async: false,
        success: function (result) {
            if (result.code=="0") {
                alert(result.message);
                window.location.href = "/admin/rs/getCountrysideList.json";
            } else {
                alert(result.message);
            }
        }
...
function updatecountryside() {
    ...
    jQuery.ajax({
        url: "/admin/rs/updateCountryside.json",
        data: date,
        type: "post",
        dataType: "json",
        async: false,
...
function delCountryside(countrysideId) {
    if (confirm("删除后将无法恢复，是否继续")) {
        jQuery.ajax({
            url: "/admin/rs/deleteCountryside.json?id="+countrysideId,
            data: {},
            type: "post",
            dataType: "json",
            async: false,
