...
public class CheckWorkAttendanceBiz extends BaseBiz<CheckWorkAttendance, CheckWorkAttendanceDao> {
     ...
     * @return:
     */
    public void importAttendanceSheet(MultipartFile myfile, Long departmentId, String attendanceName, String attendanceTime) throws Exception {
        DateFormat format1 = new SimpleDateFormat("yyyy-MM");
        Date parseDate = format1.parse(attendanceTime);
        ...
        for (int i = 2; i <= rows - 2; i++) {
            ...
            isName(userName, year + "年" + month + "月");
            Long userId = 0L;
            if (!"".equals(userName)) {
    ...
    public Map<String, Object> queryAttendanceList(CheckWorkAttendance checkWorkAttendance) {
        ...
        if (ObjectUtils.isNotNull(checkWorkAttendances)) {
            for (CheckWorkAttendance workAttendance : checkWorkAttendances) {
                //查询上午
                List<CheckWorkAttendance> amCheckWorkAttendanceList = this.find(null, "1=1  and userName=" + "'" + workAttendance.getUserName() + "'" + " and amAttendanceStatus!=0" + " and left(addTime," + checkWorkAttendance.getAddTime().length() + ") =" + "\'" + checkWorkAttendance.getAddTime() + "\'");
                ...
                List<CheckWorkAttendance> pmCheckWorkAttendanceList = this.find(null, "1=1  and userName=" + "'" + workAttendance.getUserName() + "'" + " and pmAttendanceStatus!=0" + " and left(addTime," + checkWorkAttendance.getAddTime().length() + ") =" + "\'" + checkWorkAttendance.getAddTime() + "\'");
    ...
    public void isName(String userName, String date) {
        String whereSql = " 1=1 and userName=" + "\'" + userName + "\' and left(addTime," + date.length() + ") =" + "\'" + date + "\'";
        List<CheckWorkAttendance> checkWorkAttendanceList = this.find(null, whereSql);
        if (ObjectUtils.isNotNull(checkWorkAttendanceList)) {
            String ids = checkWorkAttendanceList.stream().distinct().map(e -> e.getId().toString()).collect(Collectors.joining(","));
            List<Long> wIds = Arrays.asList(ids.split(",")).stream().distinct().map(e -> Long.valueOf(e)).collect(Collectors.toList());
            this.deleteByIds(wIds);
        }
    }
}
