...
public class ClassUtils {
	...
	public static String getUpperFirstLetterSimpleClassName(String className){
		if(StringUtils.isEmpty(className)) return "";
		String[] parts = className.split("\\.");
		String result = parts[parts.length-1];
		return result.substring(0,1) + (result.length()>1? result.substring(1):"");
	}
	...
	public static <T> T cloneObject(T entity){
		...
		try {
			...
			if(t2 != null){
				try {t2.close();} catch (IOException e) {}
			}
			if(out != null){
